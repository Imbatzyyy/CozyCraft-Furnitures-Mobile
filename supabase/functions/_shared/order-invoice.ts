...
export function invoiceEmail(order: InvoiceOrder) {
  ...
  return {
    ...
    html:
      `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="utf-8"></head><body style="margin:0;background:#f2efe8;color:#292a24;font-family:Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td style="padding:24px 12px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;margin:auto;background:#fffdf9;border:1px solid #dfd8cc;border-radius:20px;overflow:hidden"><tr><td style="padding:30px 24px;background:#30392d;color:#faf6ec"><div style="font-size:12px;letter-spacing:2px;color:#d6c7a6">COZYCRAFT FURNITURES</div><h1 style="font:normal 36px Georgia,serif;margin:20px 0 8px">Made for your home.</h1><p style="margin:0;color:#e0ddcf;line-height:1.6">Your delivered order, thoughtfully documented.</p></td></tr><tr><td style="padding:28px 24px"><p style="font-size:11px;letter-spacing:2px;color:#7a715f">ORDER INVOICE · CUSTOMER COPY</p><h2 style="font:normal 27px Georgia,serif;margin:10px 0;overflow-wrap:anywhere">${
        escape(number)
