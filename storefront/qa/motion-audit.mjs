...
try {
  for (const width of [320, 390, 768]) {
    ...
    await page.locator(".cozy-launch-screen").evaluate(el => el.classList.add("cozy-launch-screen--exiting"))
    assert.equal(await page.locator(".cozy-launch-screen > .cozy-loader").evaluate(el => getComputedStyle(el).animationName), "cozy-sofa-exit")
    assert.equal(await page.locator(".cozy-launch-screen > .cozy-loader").evaluate(el => getComputedStyle(el).animationDuration), "0.8s")
    await page.locator(".cozy-launch-screen").evaluate(el => el.classList.remove("cozy-launch-screen--exiting"))
