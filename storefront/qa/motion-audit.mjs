...
try {
  for (const width of [320, 390, 768]) {
    ...
    const body = page.locator(".cozy-loader__body").first()
    ...
    assert.equal(await page.locator(".cozy-loader__cushions").first().evaluate(el => getComputedStyle(el).animationName), "none")
    assert.equal(await body.evaluate(el => getComputedStyle(el).animationName), "cozy-sofa-breathe", "Economy mode retains lightweight loading feedback")
