...
for (const [engine, platform] of [[chromium, 'android'], [webkit, 'ios']]) {
  ...
  for (const [width, height] of [[320,640],[390,844],[768,1024],[844,390]]) {
    for (const text of ['standard','comfortable','large','extra-large']) {
      ...
      await page.getByRole('status').filter({ hasText: 'reward added' }).waitFor()
      assert.equal(await page.locator('.hc-points').textContent(), '550')
      assert.equal(await page.locator('.hc-ledger li').count(), 5)
      await page.getByRole('button', { name: 'Next', exact:true }).click()
      await page.getByRole('button', { name: 'Next', exact:true }).click()
      assert.equal(await page.locator('.hc-ledger li').count(), 2)
      assert.equal(await page.getByRole('button', { name: 'Next', exact:true }).isDisabled(), true)
      if (platform === 'ios' && width === 390 && text === 'comfortable') await page.locator('.hc-pagination').screenshot({ path: '/tmp/home-circle-pagination.png' })
