...
function Account({
  ...
  const [reviewSuccess, setReviewSuccess] = useState<{ productName: string; rating: number } | null>(null)
                    ...
                    <section className="order-review-dialog" role="dialog" aria-modal="true" aria-label={`Review ${reviewLine.product.name}`}>
                      <form onSubmit={async (event) => {
                        ...
                        try {
                          ...
                          setReviewSuccess({ productName: reviewLine.product.name, rating: Number(result?.rating || orderReviewRating) })
                          setReviewLine(null)
                          setOrderReviewRating(5)
                          setOrderReviewBody("")
                          setOrderReviewImages([])
